<div class="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow border border-slate-300 overflow-hidden">
    ...
    <div class="p-4 pb-3 border-b border-slate-300">
        <div class="flex items-start gap-3">
            ...
            <div class="flex-1">
                <h3
                    class="text-lg font-semibold text-slate-900 {{ $task->status === 'completed' ? 'line-through text-slate-500' : '' }}">
                    {{ $task->title }}
                </h3>
                @if ($task->description)
    ...
    <div class="p-4 flex flex-col gap-2">
        ...
        <div class="grid grid-cols-2 gap-2">
            ...
            <form action="{{ route('tasks.destroy', $task) }}" method="POST"
                onsubmit="return confirm('⚠️ Tem certeza que deseja remover esta tarefa?');" class="w-full">
                @csrf
                @method('DELETE')
                <button type="submit"
                    ...
                    <x-heroicon-o-trash class="w-6 h-6" /> Remover
                </button>
            </form>
        </div>
    </div>
