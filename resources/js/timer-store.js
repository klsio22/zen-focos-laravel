(() => {
  ...
  const DEFAULT = { taskId: null, duration: 25, remaining: 25 * 60, isPaused: false, startTime: null, pausedList: [] };

  const save = (s) => { window[STORE_KEY] = s; };
  const load = () => window[STORE_KEY] || { ...DEFAULT };
  const savePausedList = (list) => { window[PAUSE_STORE_KEY] = list; };
  const loadPausedList = () => window[PAUSE_STORE_KEY] || [];

  globalThis.timerStore = {
    get() { return load(); },
    getPausedList() { return loadPausedList(); },
    ...
    set(payload) {
      ...
      if (Array.isArray(payload.paused) && payload.paused.length > 0) { state.pausedList = payload.paused; savePausedList(payload.paused); } else { state.pausedList = []; savePausedList([]); }

      save(state);
      globalThis.dispatchEvent(new CustomEvent('timer-store-updated', { detail: state }));
      return state;
    },
    ...
    tick() {
      ...
      save(s);
      globalThis.dispatchEvent(new CustomEvent('timer-store-updated', { detail: s }));
    },
    ...
    reset() {
      save({ ...DEFAULT });
      savePausedList([]);
      globalThis.dispatchEvent(new CustomEvent('timer-store-updated', { detail: { ...DEFAULT } }));
    },

    subscribe(callback) { globalThis.addEventListener('timer-store-updated', (e) => { callback(e.detail); }); },
  };
})();
